...
function ResponsiveDrawer(props) {
    ...
    const getEvents = async (country, sort, categories, start) => {
        ...
        selectedCategories.length === 0
            ...
            : (url = `https://app.ticketmaster.eu/amplify/v2/events?apikey=3emDiWvgsjWAX84KicT04Sibk9XAsX88&domain=${country}&lang=en-us&category_ids=${selectedCategories.join(
                  '%2c'
              )}&sort_by=${sort}&start=${start}&rows=12`)
    ...
    const eventsList = context.events.map((event) => {
        return context.view === 'grid' ? (
            <GridCard
                ...
                image={
                    event.images && event.images.large && event.images.large.url
                        ? event.images.large.url
                        : 'https://martialartsplusinc.com/wp-content/uploads/2017/04/default-image-720x530.jpg'
                }
                key={event.id}
